...
| `demo_tunnel.sh` | Brings up the interim public demo: the container on `127.0.0.1:7860` plus a Cloudflare quick tunnel (no account, no card). Mounts **only** `storage/pages/small`, because `/pages` is an unauthenticated mount of the *parent* directory and `HARRISON_PAGE_FULL_RES=false` does not stop StaticFiles serving the 3.8 GB of full-res scans. Pins `FORWARDED_ALLOW_IPS` to the bridge gateway rather than `*`, or a spoofed `X-Forwarded-For` defeats the rate limiter. Aborts without opening the tunnel if `/pages/full/` does not 404. |
...
| `stage_corpus.sh` / `fetch_corpus.py` | **See `docs/CORPUS.md`** — the corpus is not in this repo; that document is the retrieval path for a deploy or a fresh clone. |
