...
graph TD
    ...
    H --> I["5. Score Filtering (Threshold -3.0)"]
...
| :--- | :--- | :--- | :--- |
| **Pre-Retrieval Gatekeeper** | Query expansion, medical classification | Dynamic Gemini backup model | Google Gen AI |
| **Response Generation** | Core medical answer synthesis | Dynamic Gemini production model | Google Gen AI |
| **Verification Pass** | Post-generation factual grounding | Dynamic Gemini production model | Google Gen AI |
| **Semantic Embedding** | Caching and dense retrieval indexing | `BAAI/bge-m3` (1024 dimensions) | SentenceTransformers (Local) |
...
* **Model Name:** Dynamic Gemini backup model (optimized for low latency)
...
* **Model Name:** Dynamic Gemini production model
* **Temperature:** `0.2`
* **Max Target Tokens:**
  * `smart_summary` mode: `SMART_SUMMARY_MAX_TOKENS` (default `3000`)
  * `qa` mode: `QA_MAX_TOKENS` (default `3000`)
* **Context Budget:** Up to `12,000` characters of fused textbook context is injected from the RAG pipeline.

#### C. Self-Consistency Verification Pass
* **Model Name:** Dynamic Gemini production model
* **Temperature:** `0.0` (Rigid compliance)
* **Max Target Tokens:** Matches the mode-specific generation limits.
...
| **1** | Pre-Retrieval Gatekeeper | [query_optimizer.py](file:///Users/shivasairahulkuntala/Developer/AI_Projects/nlp_models/Harrison/backend/agents/query_optimizer.py) | `optimize_query(raw_query)` | Determines domain scope, expands clinical abbreviations, frames the query, and classifies complexity using a Gemini backup model. |
| **2** | Low-Latency Caching | [semantic_cache.py](file:///Users/shivasairahulkuntala/Developer/AI_Projects/nlp_models/Harrison/backend/agents/semantic_cache.py) | `SemanticCache.check_cache(emb, metadata)` | Compares cosine similarity (threshold $\ge 0.95$) of BGE-M3 query embeddings only after exact runtime metadata matches. |
